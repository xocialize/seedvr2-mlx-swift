...
- **Status:** ✅ full inference path ported + parity-verified vs mflux (DiT `t_out` 2.1e-4, VAE 3.5e-3/7.2e-3, RNG/scheduler 0.0, int8 cosine 0.99997) and weights published. Remaining = host preprocess/color-correct + ForgeUpscaler integration (tiling, Export-tier conformer) → **see [`docs/FORGE-HANDOFF.md`](docs/FORGE-HANDOFF.md)**. Port detail: `docs/PORT-PLAN.md`.
